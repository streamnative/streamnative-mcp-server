...
func (b *PulsarAdminBrokersToolBuilder) BuildTools(_ context.Context, config builders.ToolBuildConfig) ([]builders.ServerTool, error) {
...
func (b *PulsarAdminBrokersToolBuilder) buildPulsarAdminBrokersTool() *mcpsdk.Tool {
	return builders.NewTool("pulsar_admin_brokers",
		builders.WithDescription("Unified tool for managing Apache Pulsar broker resources. This tool integrates multiple broker management functions, including:\n"+
		...
		builders.WithString("resource", builders.Required(),
			builders.Description("Type of resource to access, available options:\n"+
		...
		builders.WithString("operation", builders.Required(),
			builders.Description("Operation to perform, available options:\n"+
		...
		builders.WithString("clusterName",
			builders.Description("Pulsar cluster name, required for these operations:\n"+
				"- When resource=brokers, operation=list\n"+
				"- When resource=namespaces, operation=get"),
		),
		builders.WithString("brokerUrl",
			builders.Description("Broker URL, such as '127.0.0.1:8080', required for these operations:\n"+
				"- When resource=namespaces, operation=get"),
		),
		builders.WithString("configType",
			builders.Description("Configuration type, required when resource=config, operation=get, available options:\n"+
		...
		builders.WithString("configName",
			builders.Description("Configuration parameter name, required for these operations:\n"+
				"- When resource=config, operation=update\n"+
				"- When resource=config, operation=delete"),
		),
		builders.WithString("configValue",
			builders.Description("Configuration parameter value, required for these operations:\n"+
				"- When resource=config, operation=update"),
		),
	)
}
...
func (b *PulsarAdminBrokersToolBuilder) buildPulsarAdminBrokersHandler(readOnly bool) func(context.Context, *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	return func(ctx context.Context, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
		...
		case "config":
			...
			if (operation == "update" || operation == "delete") && readOnly {
				return adapter.NewErrorResult("Configuration update/delete operations not allowed in read-only mode. " +
...
func (b *PulsarAdminBrokersToolBuilder) handleBrokersResource(client cmdutils.Client, operation string, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	switch operation {
	case "list":
		clusterName, err := adapter.RequireString(request, "clusterName")
		if err != nil {
			return adapter.NewErrorResult("Missing required parameter 'clusterName'. " +
		...
		if err != nil {
			return adapter.NewErrorResult(fmt.Sprintf("Failed to get active brokers: %v. "+
				"Please verify the cluster name and ensure the Pulsar service is running.", err)), nil
		}

		brokersJSON, err := json.Marshal(brokers)
		if err != nil {
			return adapter.NewErrorResult("Failed to serialize brokers list: %v", err), nil
		}

		return adapter.NewTextResult(string(brokersJSON)), nil
	default:
		return adapter.NewErrorResult(fmt.Sprintf("Unsupported operation '%s' for brokers resource. "+
			"The only supported operation is 'list'.", operation)), nil
	}
}

// handleHealthResource handles health resource
func (b *PulsarAdminBrokersToolBuilder) handleHealthResource(client cmdutils.Client, operation string, _ *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	switch operation {
	case "get":
		//nolint:staticcheck
		err := client.Brokers().HealthCheck()
		if err != nil {
			return adapter.NewErrorResult(fmt.Sprintf("Broker health check failed: %v. "+
				"The broker might be down or experiencing issues.", err)), nil
		}
		return adapter.NewTextResult("ok"), nil
	default:
		return adapter.NewErrorResult(fmt.Sprintf("Unsupported operation '%s' for health resource. "+
			"The only supported operation is 'get'.", operation)), nil
	}
}

// handleConfigResource handles config resource
func (b *PulsarAdminBrokersToolBuilder) handleConfigResource(client cmdutils.Client, operation string, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	switch operation {
	case "get":
		configType, err := adapter.RequireString(request, "configType")
		if err != nil {
		...
		default:
			return adapter.NewErrorResult(fmt.Sprintf("Invalid config type: '%s'. "+
				"Valid types are: dynamic, runtime, internal, all_dynamic.", configType)), nil
		}

		if fetchErr != nil {
			return adapter.NewErrorResult("Failed to get %s configuration: %v", configType, fetchErr), nil
		}

		resultJSON, err := json.Marshal(result)
		if err != nil {
			return adapter.NewErrorResult("Failed to serialize configuration: %v", err), nil
		}

		return adapter.NewTextResult(string(resultJSON)), nil

	case "update":
		configName, err := adapter.RequireString(request, "configName")
		if err != nil {
			return adapter.NewErrorResult("Missing required parameter 'configName'. " +
				"Please provide the name of the configuration parameter to update."), nil
		}

		configValue, err := adapter.RequireString(request, "configValue")
		if err != nil {
			return adapter.NewErrorResult("Missing required parameter 'configValue'. " +
		...
		if err != nil {
			return adapter.NewErrorResult(fmt.Sprintf("Failed to update configuration: %v. "+
				"Please verify the configuration name is valid and the value is of the correct type.", err)), nil
		}

		return adapter.NewTextResult(fmt.Sprintf("Dynamic configuration '%s' updated successfully to '%s'",
	...
	case "delete":
		...
		if err != nil {
			return adapter.NewErrorResult(fmt.Sprintf("Failed to delete configuration: %v. "+
				"Please verify the configuration name is valid and exists.", err)), nil
		}

		return adapter.NewTextResult(fmt.Sprintf("Dynamic configuration '%s' deleted successfully", configName)), nil

	default:
		return adapter.NewErrorResult(fmt.Sprintf("Unsupported operation '%s' for config resource. "+
			"Supported operations are: get, update, delete.", operation)), nil
	}
}

// handleNamespacesResource handles namespaces resource
func (b *PulsarAdminBrokersToolBuilder) handleNamespacesResource(client cmdutils.Client, operation string, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	switch operation {
	case "get":
		clusterName, err := adapter.RequireString(request, "clusterName")
		if err != nil {
		...
		brokerURL, err := adapter.RequireString(request, "brokerUrl")
		if err != nil {
			return adapter.NewErrorResult("Missing required parameter 'brokerUrl'. " +
		...
		if err != nil {
			return adapter.NewErrorResult(fmt.Sprintf("Failed to get owned namespaces: %v. "+
				"Please verify the cluster name and broker URL are correct.", err)), nil
		}

		namespacesJSON, err := json.Marshal(namespaces)
		if err != nil {
			return adapter.NewErrorResult("Failed to serialize namespaces: %v", err), nil
		}

		return adapter.NewTextResult(string(namespacesJSON)), nil

	default:
		return adapter.NewErrorResult(fmt.Sprintf("Unsupported operation '%s' for namespaces resource. "+
			"The only supported operation is 'get'.", operation)), nil
	}
}
