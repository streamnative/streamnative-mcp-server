...
func (b *PulsarAdminSourcesToolBuilder) BuildTools(_ context.Context, config builders.ToolBuildConfig) ([]builders.ServerTool, error) {
...
func (b *PulsarAdminSourcesToolBuilder) buildSourcesTool() *mcpsdk.Tool {
	...
	return builders.NewTool("pulsar_admin_sources",
		...
		builders.WithString("namespace",
			builders.Description("The namespace name. Namespaces are logical groupings of topics and sources "+
		...
		builders.WithString("name",
			builders.Description("The source name. Required for all operations except 'list' and 'list-built-in'. "+
		...
		builders.WithString("archive",
			builders.Description("Path to the archive file containing the source code. Optional for 'create' and 'update' operations. "+
		...
		builders.WithString("source-type",
			builders.Description("The built-in source connector type to use. Optional for 'create' and 'update' operations. "+
		...
		builders.WithString("destination-topic-name",
			builders.Description("The Pulsar topic to which data is published. Required for 'create' operation, optional for 'update'. "+
		...
		builders.WithString("deserialization-classname",
			builders.Description("The SerDe (Serialization/Deserialization) classname for the source. Optional for 'create' and 'update'. "+
		...
		builders.WithString("schema-type",
			builders.Description("The schema type to be used to encode messages emitted from the source. Optional for 'create' and 'update'. "+
		...
		builders.WithString("classname",
			builders.Description("The source's class name if archive is a file-url-path (file://...). Optional for 'create' and 'update'. "+
		...
		builders.WithString("processing-guarantees",
			builders.Description("The processing guarantees (delivery semantics) applied to the source. Optional for 'create' and 'update'. "+
		...
		builders.WithNumber("parallelism",
			builders.Description("The parallelism factor of the source. Optional for 'create' and 'update' operations. "+
		...
		builders.WithObject("source-config",
			builders.Description("User-defined source config key/values. Optional for 'create' and 'update' operations. "+
...
func (b *PulsarAdminSourcesToolBuilder) buildSourcesHandler(readOnly bool) func(context.Context, *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	return func(ctx context.Context, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
		...
		if readOnly && writeOperations[operation] {
			return adapter.NewErrorResult("Operation '%s' not allowed in read-only mode. Read-only mode restricts modifications to Pulsar Sources.", operation), nil
		}
		...
		if operation != "list" {
			...
			if err != nil {
				return adapter.NewErrorResult("Missing required parameter 'name' for operation '%s': %v. The source name must be specified for this operation.", operation, err), nil
			}
		}
...
func (b *PulsarAdminSourcesToolBuilder) handleSourceList(_ context.Context, admin cmdutils.Client, tenant, namespace string) (*mcpsdk.CallToolResult, error) {
	sources, err := admin.Sources().ListSources(tenant, namespace)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to list sources in tenant '%s' namespace '%s': %v. Check that the tenant and namespace exist and you have proper permissions.",
	...
	if err != nil {
		return adapter.NewErrorResult("Failed to serialize source list: %v", err), nil
	}

	return adapter.NewTextResult(string(sourcesJSON)), nil
}

// handleSourceGet handles getting information about a source
func (b *PulsarAdminSourcesToolBuilder) handleSourceGet(_ context.Context, admin cmdutils.Client, tenant, namespace, name string) (*mcpsdk.CallToolResult, error) {
	source, err := admin.Sources().GetSource(tenant, namespace, name)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to get source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and you have proper permissions.",
	...
	if err != nil {
		return adapter.NewErrorResult("Failed to serialize source info: %v", err), nil
	}

	return adapter.NewTextResult(string(sourceJSON)), nil
}

// handleSourceStatus handles getting the status of a source
func (b *PulsarAdminSourcesToolBuilder) handleSourceStatus(_ context.Context, admin cmdutils.Client, tenant, namespace, name string) (*mcpsdk.CallToolResult, error) {
	status, err := admin.Sources().GetSourceStatus(tenant, namespace, name)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to get status for source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and is properly deployed.",
			name, tenant, namespace, err)), nil
	}
	...
	if err != nil {
		return adapter.NewErrorResult("Failed to serialize source status: %v", err), nil
	}
...
func (b *PulsarAdminSourcesToolBuilder) handleSourceCreate(_ context.Context, admin cmdutils.Client, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	tenant, err := adapter.RequireString(request, "tenant")
	if err != nil {
	...
	sourceType := adapter.GetString(request, "source-type",  "")
	if sourceType != "" {
		sourceData.SourceType = sourceType
	}

	destTopic := adapter.GetString(request, "destination-topic-name",  "")
	if destTopic != "" {
		sourceData.DestinationTopicName = destTopic
	}

	deserializationClassName := adapter.GetString(request, "deserialization-classname",  "")
	if deserializationClassName != "" {
		sourceData.DeserializationClassName = deserializationClassName
	}

	schemaType := adapter.GetString(request, "schema-type",  "")
	if schemaType != "" {
		sourceData.SchemaType = schemaType
	}

	className := adapter.GetString(request, "classname",  "")
	if className != "" {
		sourceData.ClassName = className
	}

	processingGuarantees := adapter.GetString(request, "processing-guarantees",  "")
	...
	sourceConfigMap := adapter.GetObject(request, "source-config")
	if sourceConfigMap != nil {
		// Convert to JSON string
		sourceConfigJSON, err := json.Marshal(sourceConfigMap)
		if err != nil {
			return adapter.NewErrorResult("Failed to marshal source-config: %v. Ensure the source configuration is a valid JSON object.", err), nil
		}
	...
	if sourceData.Archive == "" && sourceData.SourceType == "" {
		return adapter.NewErrorResult("Missing required parameter: Either 'archive' or 'source-type' must be specified for source creation. Use 'archive' for custom connectors or 'source-type' for built-in connectors."), nil
	}

	if sourceData.Archive != "" && sourceData.SourceType != "" {
		return adapter.NewErrorResult("Invalid parameters: Cannot specify both 'archive' and 'source-type'. Use only one of these parameters based on your connector type."), nil
	}

	if sourceData.DestinationTopicName == "" {
		return adapter.NewErrorResult("Missing required parameter: 'destination-topic-name' must be specified. This is the Pulsar topic where the source will publish data."), nil
	}
	...
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to create source '%s' in tenant '%s' namespace '%s': %v. Verify all parameters are correct and required resources exist.",
			name, tenant, namespace, err)), nil
	}

	return adapter.NewTextResult(fmt.Sprintf("Created source '%s' successfully in tenant '%s' namespace '%s'. The source will start pulling data from the external system and publishing to the destination topic.",
		name, tenant, namespace)), nil
}

// handleSourceUpdate handles updating an existing source
func (b *PulsarAdminSourcesToolBuilder) handleSourceUpdate(_ context.Context, admin cmdutils.Client, request *mcpsdk.CallToolRequest) (*mcpsdk.CallToolResult, error) {
	tenant, err := adapter.RequireString(request, "tenant")
	if err != nil {
		return adapter.NewErrorResult("Failed to get tenant: %v", err), nil
	}
	...
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to update source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and all parameters are valid.",
			name, tenant, namespace, err)), nil
	}

	return adapter.NewTextResult(fmt.Sprintf("Updated source '%s' successfully in tenant '%s' namespace '%s'. The source may need to be restarted to apply all changes.",
		name, tenant, namespace)), nil
}

// handleSourceDelete handles deleting a source
func (b *PulsarAdminSourcesToolBuilder) handleSourceDelete(_ context.Context, admin cmdutils.Client, tenant, namespace, name string) (*mcpsdk.CallToolResult, error) {
	err := admin.Sources().DeleteSource(tenant, namespace, name)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to delete source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and you have deletion permissions.",
			name, tenant, namespace, err)), nil
	}

	return adapter.NewTextResult(fmt.Sprintf("Deleted source '%s' successfully from tenant '%s' namespace '%s'. All running instances have been terminated.",
		name, tenant, namespace)), nil
}

// handleSourceStart handles starting a source
func (b *PulsarAdminSourcesToolBuilder) handleSourceStart(_ context.Context, admin cmdutils.Client, tenant, namespace, name string) (*mcpsdk.CallToolResult, error) {
	err := admin.Sources().StartSource(tenant, namespace, name)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to start source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and is not already running.",
			name, tenant, namespace, err)), nil
	}

	return adapter.NewTextResult(fmt.Sprintf("Started source '%s' successfully in tenant '%s' namespace '%s'. The source will begin pulling data from the external system.",
		name, tenant, namespace)), nil
}

// handleSourceStop handles stopping a source
func (b *PulsarAdminSourcesToolBuilder) handleSourceStop(_ context.Context, admin cmdutils.Client, tenant, namespace, name string) (*mcpsdk.CallToolResult, error) {
	err := admin.Sources().StopSource(tenant, namespace, name)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to stop source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and is currently running.",
			name, tenant, namespace, err)), nil
	}

	return adapter.NewTextResult(fmt.Sprintf("Stopped source '%s' successfully in tenant '%s' namespace '%s'. The source will no longer pull data until restarted.",
		name, tenant, namespace)), nil
}

// handleSourceRestart handles restarting a source
func (b *PulsarAdminSourcesToolBuilder) handleSourceRestart(_ context.Context, admin cmdutils.Client, tenant, namespace, name string) (*mcpsdk.CallToolResult, error) {
	err := admin.Sources().RestartSource(tenant, namespace, name)
	if err != nil {
		return adapter.NewErrorResult(fmt.Sprintf("Failed to restart source '%s' in tenant '%s' namespace '%s': %v. Verify the source exists and is properly deployed.",
			name, tenant, namespace, err)), nil
	}

	return adapter.NewTextResult(fmt.Sprintf("Restarted source '%s' successfully in tenant '%s' namespace '%s'. All source instances have been restarted.",
		name, tenant, namespace)), nil
}

// handleListBuiltInSources handles listing all built-in source connectors
func (b *PulsarAdminSourcesToolBuilder) handleListBuiltInSources(_ context.Context, admin cmdutils.Client) (*mcpsdk.CallToolResult, error) {
	sources, err := admin.Sources().GetBuiltInSources()
	if err != nil {
		return adapter.NewErrorResult("Failed to list built-in sources: %v. There might be an issue connecting to the Pulsar cluster.", err), nil
	}

	// Convert result to JSON string
	sourcesJSON, err := json.Marshal(sources)
	if err != nil {
		return adapter.NewErrorResult("Failed to serialize built-in sources: %v", err), nil
	}
