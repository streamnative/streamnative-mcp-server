...
func TestPulsarAdminBrokerStatsToolBuilder(t *testing.T) {
	builder := NewPulsarAdminBrokerStatsToolBuilder()

	t.Run("GetName", func(t *testing.T) {
		assert.Equal(t, "pulsar_admin_broker_stats", builder.GetName())
	})

	t.Run("GetRequiredFeatures", func(t *testing.T) {
		features := builder.GetRequiredFeatures()
		assert.NotEmpty(t, features)
		assert.Contains(t, features, "pulsar-admin-brokers-status")
	})
	...
	t.Run("BuildTools_Success", func(t *testing.T) {
		...
		assert.Equal(t, "pulsar_admin_broker_stats", tools[0].Definition().Name)
	})
...
func TestPulsarAdminBrokerStatsToolSchema(t *testing.T) {
	builder := NewPulsarAdminBrokerStatsToolBuilder()
	tool, err := builder.buildBrokerStatsTool()
	require.NoError(t, err)
	assert.Equal(t, "pulsar_admin_broker_stats", tool.Name)
	...
	expectedProps := []string{"resource", "allocator_name"}
	...
	assert.Equal(t, pulsarAdminBrokerStatsResourceDesc, resourceSchema.Description)

	allocatorSchema := schema.Properties["allocator_name"]
	require.NotNil(t, allocatorSchema)
	assert.Equal(t, pulsarAdminBrokerStatsAllocatorNameDesc, allocatorSchema.Description)
}
