...
func TestPulsarAdminTenantToolBuilder(t *testing.T) {
	builder := NewPulsarAdminTenantToolBuilder()

	t.Run("GetName", func(t *testing.T) {
		assert.Equal(t, "pulsar_admin_tenant", builder.GetName())
	})

	t.Run("GetRequiredFeatures", func(t *testing.T) {
		features := builder.GetRequiredFeatures()
		assert.NotEmpty(t, features)
		assert.Contains(t, features, "pulsar-admin-tenants")
	})

	t.Run("BuildTools_Success", func(t *testing.T) {
		config := builders.ToolBuildConfig{
			ReadOnly: false,
			Features: []string{"pulsar-admin-tenants"},
		}

		tools, err := builder.BuildTools(context.Background(), config)
		require.NoError(t, err)
		assert.Len(t, tools, 1)
		assert.Equal(t, "pulsar_admin_tenant", tools[0].Definition().Name)
...
func TestPulsarAdminTenantToolSchema(t *testing.T) {
	builder := NewPulsarAdminTenantToolBuilder()
	tool, err := builder.buildTenantTool()
	require.NoError(t, err)
	assert.Equal(t, "pulsar_admin_tenant", tool.Name)
	...
	assert.Equal(t, pulsarAdminTenantResourceDesc, resourceSchema.Description)

	operationSchema := schema.Properties["operation"]
	require.NotNil(t, operationSchema)
	assert.Equal(t, pulsarAdminTenantOperationDesc, operationSchema.Description)
}

func TestPulsarAdminTenantToolBuilder_ReadOnlyRejectsWrite(t *testing.T) {
	builder := NewPulsarAdminTenantToolBuilder()
	handler := builder.buildTenantHandler(true)

	_, _, err := handler(context.Background(), nil, pulsarAdminTenantInput{
		Resource:  "tenant",
		Operation: "create",
	})
