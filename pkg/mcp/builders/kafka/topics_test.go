...
func TestKafkaTopicsToolBuilder(t *testing.T) {
	builder := NewKafkaTopicsToolBuilder()

	t.Run("GetName", func(t *testing.T) {
		assert.Equal(t, "kafka_topics", builder.GetName())
	})
	...
	t.Run("BuildTools_Success", func(t *testing.T) {
		...
		assert.Equal(t, "kafka_admin_topics", tools[0].Definition().Name)
...
func TestKafkaTopicsToolSchema(t *testing.T) {
	builder := NewKafkaTopicsToolBuilder()
	tool, err := builder.buildKafkaTopicsTool()
	require.NoError(t, err)
	assert.Equal(t, "kafka_admin_topics", tool.Name)
	...
	assert.Equal(t, kafkaTopicsResourceDesc, resourceSchema.Description)

	operationSchema := schema.Properties["operation"]
	require.NotNil(t, operationSchema)
	assert.Equal(t, kafkaTopicsOperationDesc, operationSchema.Description)
}

func TestKafkaTopicsToolBuilder_ReadOnlyRejectsWrite(t *testing.T) {
	builder := NewKafkaTopicsToolBuilder()
	handler := builder.buildKafkaTopicsHandler(true)

	_, _, err := handler(context.Background(), nil, kafkaTopicsInput{
		Resource:  "topic",
		Operation: "create",
	})
