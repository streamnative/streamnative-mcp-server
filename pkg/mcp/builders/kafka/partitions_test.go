...
func TestKafkaPartitionsToolBuilder(t *testing.T) {
	builder := NewKafkaPartitionsToolBuilder()

	t.Run("GetName", func(t *testing.T) {
		assert.Equal(t, "kafka_partitions", builder.GetName())
	})
	...
	t.Run("BuildTools_Success", func(t *testing.T) {
		...
		assert.Equal(t, "kafka_admin_partitions", tools[0].Definition().Name)
...
func TestKafkaPartitionsToolSchema(t *testing.T) {
	builder := NewKafkaPartitionsToolBuilder()
	tool, err := builder.buildKafkaPartitionsTool()
	require.NoError(t, err)
	assert.Equal(t, "kafka_admin_partitions", tool.Name)
	...
	assert.Equal(t, kafkaPartitionsResourceDesc, resourceSchema.Description)

	operationSchema := schema.Properties["operation"]
	require.NotNil(t, operationSchema)
	assert.Equal(t, kafkaPartitionsOperationDesc, operationSchema.Description)
}

func TestKafkaPartitionsToolBuilder_ReadOnlyRejectsWrite(t *testing.T) {
	builder := NewKafkaPartitionsToolBuilder()
	handler := builder.buildKafkaPartitionsHandler(true)

	_, _, err := handler(context.Background(), nil, kafkaPartitionsInput{
		Resource:  "partition",
		Operation: "update",
	})
