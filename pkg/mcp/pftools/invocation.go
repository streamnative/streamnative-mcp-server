...
func (fi *FunctionInvoker) InvokeFunctionAndWait(ctx context.Context, fnTool *FunctionTool, params map[string]interface{}) (*mcpsdk.CallToolResult, error) {
	schemaConverter, err := schema.ConverterFactory(fnTool.OutputSchema.Type)
	if err != nil {
		return &mcpsdk.CallToolResult{
			Content: []mcpsdk.Content{
				&mcpsdk.TextContent{
					Text: fmt.Sprintf("Failed to get schema converter: %v", err),
				},
			},
			IsError: true,
		}, nil
	...
	if err != nil {
		return &mcpsdk.CallToolResult{
			Content: []mcpsdk.Content{
				&mcpsdk.TextContent{
					Text: fmt.Sprintf("Failed to serialize payload: %v", err),
				},
			},
			IsError: true,
		}, nil
	...
	if err != nil {
		return &mcpsdk.CallToolResult{
			Content: []mcpsdk.Content{
				&mcpsdk.TextContent{
					Text: fmt.Sprintf("Failed to set up consumer: %v", err),
				},
			},
			IsError: true,
		}, nil
	...
	case result := <-resultChan:
		if result.Error != nil {
			return &mcpsdk.CallToolResult{
				Content: []mcpsdk.Content{
					&mcpsdk.TextContent{
						Text: fmt.Sprintf("Function execution failed: %v", result.Error),
					},
				},
				IsError: true,
			}, nil
	...
	case <-ctx.Done():
		return &mcpsdk.CallToolResult{
			Content: []mcpsdk.Content{
				&mcpsdk.TextContent{
					Text: fmt.Sprintf("Function invocation timed out after %v", ctx.Value("timeout")),
				},
			},
			IsError: true,
		}, nil
