...
func (o *ServerOptions) AddFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().BoolVar(&o.MultiSessionPulsar, "multi-session-pulsar", false, "Enable per-user Pulsar sessions based on Authorization header tokens (only for external Pulsar mode)")
	cmd.PersistentFlags().IntVar(&o.SessionCacheSize, "session-cache-size", 100, "Maximum number of cached Pulsar sessions when multi-session is enabled")
	cmd.PersistentFlags().IntVar(&o.SessionTTLMinutes, "session-ttl-minutes", 30, "Session TTL in minutes before eviction when multi-session is enabled")
}
