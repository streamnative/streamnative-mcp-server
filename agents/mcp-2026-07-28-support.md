...
### Current profile hardening — 2026-09-18 (mcp-go v1.1.0)

| CLI profile | Protocol policy | Scope |
|---|---|---|
| `http`, fixed external Kafka/Pulsar | Modern-only `2026-07-28` | Static catalog; Pulsar may use request-scoped backend credentials |
| `stdio`, exactly one fixed external backend | Legacy and modern | Initial tenant wire tests plus profile/dispatch regression tests; not all tools certified |
| `stdio`, Cloud or other state-dependent configuration | Legacy operations only | Modern discovery remains available to negotiate down; modern operations are rejected before dispatch |
| `sse` | Legacy HTTP+SSE | Existing runtime and capabilities unchanged |

Stdio v1.1.0 does not enforce the supported-version context during dispatch.
The CLI therefore uses an initialization hook to reject modern operations for
legacy-only profiles, including direct calls before discovery. The SDK maps
these hook failures to `INVALID_REQUEST`; no SDK fork is used. Discovery returns
only legacy versions for those profiles. Legacy initialize still negotiates
normally, including after a rejected modern call.

For modern-enabled profiles, modern discovery no longer advertises resource
`subscribe`/`listChanged`. Discovery of a legacy-only profile reports its legacy
capabilities so clients can negotiate down.
Modern resource subscription filters are declined (excluded from the requested
filter); empty subscriptions complete immediately. Stdio applies this to each
modern request/response, without mutating the capabilities used by legacy
initialize. The HTTP-only server configures its own capabilities before serving.
Request-scoped progress streaming is unaffected. Resource change delivery and
full subscription conformance remain outside the supported profile.

In multi-session mode, HTTP gives each bearer-token request a private Pulsar
session, released once after the response completes. It has no per-request
LRU/TTL manager or cleanup goroutine. Only the request token and allowed backend/TLS settings enter that
session; global tokens, auth plugins, and client certificates are not inherited.
Shared leased connection pooling remains future work.

Downstream embedders such as `mcp-auth0-proxy` keep the existing builders,
context helpers and server defaults. They must explicitly pin their own
Streamable HTTP constructors to `LegacyProtocolVersions()` until their dynamic
catalogs, context switching and audit paths are migrated. Updating this CLI
does not configure a downstream server's transport.

Acceptance includes real stdio pipe tests of direct-call rejection, discovery
and legacy recovery; modern HTTP capability/filter tests; request credential
isolation and lifetime tests. These do not replace Docker/Kind/backend E2E or
independent-client interoperability testing. Historical validation below applies
only to its dated snapshot, not to this increment.

Validation for this increment (Go 1.25.14): dependency verify/download,
formatting/tidy, lint (0 issues), `go test -race ./... -count=1` (617 tests),
`make build`, license headers, 39 E2E harness contract checks, and chart
transport rendering/lint passed. No Kind clusters were available locally;
full Docker/Kind/backend E2E was not run and remains a release gate.

### Historical Stage 2 increment — 2026-09-08

The new `http` command mounts a modern-only POST endpoint at `/mcp` with
unauthenticated health probes. It defaults to `127.0.0.1:9090`, accepts exactly
one external Kafka/Pulsar backend and rejects Cloud configuration before
credential setup. It never registers session-scoped Functions-as-tools.
Legacy SSE and stdio remain unchanged. The chart opts in with
`server.transport=http`; missing transport in reused old values means SSE.

Wire tests cover discovery/direct calls, JSON/request-scoped SSE, headers and
metadata, removed methods, no session ID, body size, Origin and credential
rejection, and request-scoped credential isolation/release. Incoming bodies are
limited to 4 MiB, production body reads to 30 seconds; no response write timeout
is imposed on SSE. Exact Origins may be configured; default is deny all present
Origins. This does not implement browser CORS preflight.

Non-loopback requires external Pulsar multi-session mode. This deliberately
retains a **backend credential forwarding compatibility profile**, not an MCP
OAuth resource server. Client construction does not validate tokens and a
syntactically valid unauthorized token may discover the catalog; Pulsar checks
authorization on backend operations. A trusted authenticating TLS gateway and
network restrictions are required for remote use. No issuer/audience validation
or OAuth metadata is claimed (see S7). Fixed local backends share one identity.

The old cache can close in-flight clients during eviction. HTTP instead gives
each request a private manager/client, released after the handler returns. It
does not inherit global token/plugin/client-certificate credentials; legacy
cache flags are rejected and omitted by the HTTP chart. This trades pooling
efficiency for clear lifetimes; a leased shared pool remains future work.
Shutdown cancels request contexts, drains for 10 seconds, then closes network
connections but waits for active handlers before closing backend clients.
Handlers that ignore cancellation can therefore delay process exit.

Stage 2 remains a bounded increment, **not complete acceptance**: real
multi-replica/backend authorization, broad subscription delivery, independent
client interoperability and OAuth are still unverified. The full chart E2E
attempt built its Docker image but failed loading it because the local Kind
cluster has no nodes. Helm deployment/authenticated E2E did not run.

### Stage 0/1 baseline — 2026-09-07
...
## Historical research conclusions (2026-09-07 baseline)

The remaining research and staged plan describe baseline `77134bc` and SDK
v1.0.0. Use the current implementation snapshot above for shipped behavior and
the current `go.mod` for the SDK version.
...
- The research baseline exposed **stdio and legacy HTTP+SSE**, not a
